...
## <a id="csp-meta-body"></a>머리 없는 문서에서 CSP meta 가 `<body>` 안으로 들어갔다 (2026-08-26)

스트리밍 리라이터의 프렐류드 앵커가 `head` → `body` → 첫 `script` 앞 순서였다.
`<head>` 가 없는 문서(우리는 브라우저 파서가 아니라 **원본 바이트**를 보므로
브라우저가 자동 생성해 주는 head 는 여기 없다)에서는 프렐류드가 `<body>` 안으로
들어갔다. 그런데 **프렐류드에는 CSP meta 가 들어 있고, body 안의 CSP meta 는
브라우저가 무시한다.** 스트리밍 응답에서는 CSP **헤더**도 강제되지 않는다는
것을 이미 측정해 뒀으므로(그래서 meta 를 박기 시작했다) 그 조합에서는 정책이
**하나도** 안 걸린다.

앵커 맨 앞에 `html` 을 넣었다. `<html>` 바로 뒤는 언제나 유효한 자리다 —
파서는 "before head" 상태에서 meta/script 를 만나면 head 를 암묵적으로 만들어
거기 넣고, 뒤따라 오는 진짜 `<head>` 시작 태그는 무시하고 속성만 병합한다.
즉 head 가 있든 없든 결과는 "head 안" 으로 같고 오히려 더 이르다.

실측(로컬 픽스처 `<html><body>…`, 격리 월드에서 날 DOM 을 봄):

```
{"has":true,"parent":"HEAD","inHead":true,"bodyText":"no head here…"}
```
...
## <a id="style-raw-text-이스케이프"></a>`<style>` 을 HTML 이스케이프해서 CSS 규칙 98개가 죽었다 — 자식 결합자 `>` 가 `&gt;` 로 (2026-08-26, CNN)

한 줄 요약: **`chunk.after(&rewritten, ContentType::Text)`**. `<style>` 은 raw
text 라 lol_html 이 이스케이프를 걸면 안 되는데 걸었다.

### 어떻게 여기까지 왔나 (증상은 CSS 와 아무 상관 없어 보였다)

"비디오 CDN 이 다르다(대조군 라이브 `akm.*` vs 프록시 VOD `cf.*`)" 를 쫓다가
나왔다. 그 전에 두 번 헛다리를 짚었고 둘 다 **관측 창 아티팩트**였다:

- "라이브가 5세그먼트에서 멎는다" → 창을 60초로 늘리니 t+82s 까지 계속 받는다.
  프록시는 부팅에 20초를 더 쓰므로 같은 대기로는 창이 짧다.
- "세그먼트 수가 절반이다" → 재생 자체를 재야 했다.

재생을 재니 갈렸다. 대조군 `paused=false`, `currentTime` 이 실시간과 **1:1**.
프록시 `paused=true` — 버퍼는 채우면서 재생을 안 한다.

멈춤 신호(가시성/포커스/음소거/자동재생)를 다 재도 같았고, 다른 것 하나가
남았다: **비디오의 위치**.

| | 대조군 | 프록시 |
|---|---|---|
| 라이브 플레이어 rect | (836, 403) 289×163 | (16, **5073**) 1127×634 |
| 뷰포트 안 | true | **false** |

조상 체인은 **완전히 같았다**(같은 위젯, 같은 컨테이너). 즉 다른 모듈이 아니라
**레이아웃**이다. 페이지 높이 7,760 vs **22,590px**. 화면 밖이니 플레이어가
안 트는 것은 정상 동작이었다 — 우리가 화면 밖으로 밀어낸 것이 문제다.

### 진짜 원인

`.zone__items.layout--wide-center` 의 자식 폭이 대조군 317px(3열), 프록시
1153px(전폭)이고 `gap` 도 대조군만 `32px` 였다. 그 폭을 주는 규칙을 찾으니
스타일시트 **텍스트에는 50번 나오는데 `cssRules` 에는 0번** 나왔다 = 파싱 실패.

텍스트를 보면:

```
.product-zone__items.layout--wide-center&gt;div:nth-child(3){width:calc(30% - 16px)}
```

`>` 가 `&gt;` 다. **`<style>` 안에서는 엔티티가 풀리지 않는다**(raw text
element). 그래서 그 선택자를 쓴 규칙이 통째로 버려졌다.
...
98개가 사라졌고 하필 그것이 그리드 폭 규칙이었다. 3열이 세로로 쌓이니 높이가
3배가 되고, 라이브 플레이어가 5,073px 아래로 밀려 **재생이 시작되지 않았다.**

### 고침

```rust
- chunk.after(&rewritten, ContentType::Text);   // lol_html 이 이스케이프한다
+ chunk.after(&rewritten, ContentType::Html);   // raw passthrough
```

**바로 아래 스크립트 경로가 이미 같은 이유로 `Html` 을 쓰고 있었고 주석까지
달려 있었다**(`ContentType::Html → raw passthrough (no HTML escaping)`).
인라인 CSS 리라이트를 나중에 붙이면서 그 관례를 안 따라간 것이다.

`<style>` 버퍼에 `</style` 이 들어올 수는 없다(파서가 거기서 요소를 끝낸다).
`rewrite_css` 가 만드는 프록시 URL 은 퍼센트 인코딩이라 `<` 를 못 만든다.
...
|---|---|---|---|
| `&gt;` / 생 `>` | 671 / 0 | 0 / 671 | 0 / 671 |
| CSS 규칙 | 4,546 | **4,644** | 4,644 |
| 페이지 높이 | 22,590px | **7,795px** | 7,760px |
| 플레이어 rect | y=5073, 화면 밖 | **(836,403) 289×163** | (836,403) 289×163 |
| `paused` | **true** | **false** | false |
| 재생 진행/실시간 | — | **1.00** (waiting 0) | 1.00 (waiting 0) |

회귀: zp-htmltx 57, cargo workspace 전부 통과, static 158,
naver/wikipedia/github 렌더 정상, 탐지기 0건. 변이 1/1.

### 교훈

- **요소 수가 같아도 레이아웃은 다를 수 있다.** 지금까지의 회귀 검사(요소 수,
  raw URL 수, CSP 위반, 콘솔 에러)는 이걸 전부 통과시켰다. 페이지 높이 3배가
  아무 경보도 울리지 않았다 — `rendercheck.sh` 에 **높이/뷰포트 축**이 필요하다.
- **"파싱된 규칙 수" 는 CSS 파이프라인의 좋은 오라클이다.** 텍스트 길이는 오히려
  늘어 있었다(이스케이프가 글자를 늘리므로). 길이로 봤으면 못 찾는다.
- **텍스트에는 있는데 CSSOM 에는 없다** = 파싱 실패. 이 두 눈의 차이가 원인을
  한 번에 짚어 준다.
