...
## <a id="cnn-2차정지-재현불가"></a>CNN "두 번째 정지" 는 재현되지 않는다 — 유력 후보는 반증됐다 (2026-08-26)

기록돼 있던 증상: CNN 을 프록시로 열면 **~7초에 렌더러가 먹통**. 하트비트는
그때까지 규칙적, DOM 계수기 조용(setAttr 6,463 / getAttr 4,768), 렌더러
**572MB 로 생존**, **CDP 만 무응답**.

### 재현 시도 — 9회 / 4변종 / 0건

| 변종 | 회 | 결과 |
|---|---|---|
| 첫 화면 (edition.cnn.com) | 6 | 전부 `complete`, exec-js rtt ≤1s |
| 기사 페이지 (live-news) | 1 | `complete`, rtt ≤1s |
| 콜드 스타트 (site-data 전부 삭제) | 1 | `complete`, rtt ≤1s |
| 무거운 스크롤 105초 (지연 광고 계속 깨움) | 1 | `complete`, 힙 124~193MB 에서 GC 정상 |

`--record --watch-hang` 을 켠 데몬으로 돌렸고 자동 덤프는 **0건**이다.
`performance.now()` 가 전 구간 선형이고 요소 수는 대조군과 붙는다
(프록시 4,171~4,184 vs 직접 로드 4,180). 힙은 572MB 근처에도 안 간다.

### 유력 후보를 **반증**했다

`1bcae09` 의 `await self.clients.get(resultingClientId)` 교착이 첫 번째
후보였다 — "SW 는 200 을 만들었는데 브라우저만 커밋을 못 한다" 는 그림이
"CDP 무응답" 과 비슷해 보였기 때문이다. 지금 트리에 **그 await 를 일부러
되돌려 심고** 빌드해서 재 봤다:

```
교착 빌드   t+53s  rs=interactive  els=4172  frames=24   ← complete 로 못 간다
정상 빌드   t+36s  rs=complete     els=4182  frames=25
```

렌더러는 **끝까지 응답한다**(rtt ≤1s). 즉 그 교착의 서명은 "로드가 안 끝남"
이지 "렌더러 먹통" 이 아니다. **후보에서 제외.** (실험 후 `git checkout` 으로
원복했다 — 이 변이는 절대 커밋하지 말 것.)

### 그래서 지금 할 수 있는 말

원인을 못 밝혔다. 재현이 없으니 밝힐 수도 없다. **가설**로만 남긴다: 첫 번째
정지의 원인이던 `filteredCollection` 의 O(N²) 순회(`5004b5e` 에서 메모이제이션
으로 고침)는 **할당 폭풍**을 만든다 — 572MB / DOM 계수기 조용 / JS 는 도는데
CDP 만 죽는 서명과 잘 맞는다. "두 번째" 로 세었던 것이 실은 부분적으로만
고쳐진 같은 원인이었을 가능성이 있다. **확인 안 된 가설이다.**

### 덫은 놓아 뒀다

다시 나면 손으로 쫓지 말 것. `zp` 데몬을 `--record --watch-hang
--auto-dump-dir <dir>` 로 띄워 두면 먹통 순간에 리코딩 버퍼와 `hang.json` 이
자동으로 떨어진다. 그다음은:

- `pause --symbolize --samples 6` — 한 장으로는 "지나가는 중" 과 "돌고 있음" 을
  구분 못 한다. msedge 내부 이름(V8 GC/alloc)까지 보려면 `--download-symbols`
  (`dbghelp.dll`+`symsrv.dll` 를 exe 옆에 둬야 하고 첫 심볼 ~630MB).
- 스택이 GC/예외 기계 위에 있으면 `--full-memory --only-pid <살찐 렌더러>` —
  기본 덤프엔 힙이 없어 "무엇이 할당됐나" 에 답을 못 한다.
- `debugger-arm --strategy exceptions --sample-every N` — Proxy 불변식
  `TypeError` 처럼 **엔진이 던지는** 예외는 페이지의 `Error` 를 감싸서는
  절대 안 보인다.

### 남은 실측 차이 — 프레임

정지 대신 **프레임 수**가 남았다: 프록시 25~29 vs 대조군 33~34. 요소 수는
붙었으니(4,180 근처) 문서는 다 그려지고 **광고 iframe 몇 개만** 안 뜬다.
이게 다음 표적이다.

## <a id="레이아웃-축-없음"></a>회귀 축에 레이아웃이 없어서 GitHub 이 내내 깨져 있었다 (2026-09-04)

2026-08-26 CSS 버그(`<style>` 이스케이프)를 잡고 나서 남긴 숙제를 이행한 것.
그때 돌던 축(요소 수 / raw URL / CSP / 콘솔 에러)이 **전부 통과**했고 페이지
높이가 3배인데 아무 경보도 안 울렸다.

### 넣은 축 — `rendercheck.sh` 를 **대조군과 쌍으로**

프록시 단독 수치는 뜻이 없다. 그래서 사이트마다 직접 로드 → 프록시 로드를
연달아 재고 비교한다.

| 축 | 판정 |
|---|---|
| `styleEntities` | `<style>` 텍스트의 `&gt;`/`&lt;` 개수. **0이 아니면 무조건 우리 탓** — `<style>` 은 raw text 라 엔티티가 안 풀리므로 원본에 있었다면 원본 브라우저에서도 죽어 있었을 것이다 |
| `height` 비율 | 프록시/대조군. 70~145% 밖이면 FLAG |
| `aboveFold` | 첫 화면에 실제로 보이는 요소 수. 높이만 보면 "전부 화면 밖으로 밀렸다" 를 놓친다 |

**규칙 수는 오라클로 안 쓴다.** 프록시는 전부 same-origin 이라 cross-origin
시트까지 읽히므로 총합이 오히려 더 크다(그 버그 때 6,415 vs 5,608). 정보로만 싣는다.

### 양성 대조 — 그 한 줄을 되돌려 확인했다

```
음성(고쳐진 빌드)  height 7650 / 7650 = 100%   els 4010 / 4027   styleEntities 0   => OK
양성(버그 되살림)  height 23230 / 7650 = 303%  els 4034 / 4012   styleEntities 837 => STYLE_ENTITY(837) HEIGHT(303%)
```

**요소 수는 프록시가 오히려 많고**(4034 vs 4012) `raw=0 csp=0` 도 통과다. 새 축
둘만 문다 — 왜 예전 축이 놓쳤는지가 이 한 줄에 그대로 보인다.

### 첫 실전에서 GitHub 이 걸렸다 — **내내 깨져 있었다**

4개 사이트 오탐 점검을 돌리자마자:

```
github.com   height 1718 / 10996 = 15%   els 906 / 1788   aboveFold 75 / 78   => HEIGHT(15%)
```

들여다보니 레이아웃이 아니라 **내용**이었다:

| | 대조군 | 프록시 |
|---|---|---|
| `h1` | The future of building happens together | **ErrorLooks like something went wrong!** |
| body 텍스트 | 6,045자 | 1,085자 |
| `signIn`/`signUp` 링크 | 있음 | 없음 |

GitHub 이 자기 **에러 페이지**를 그리고 있는데 `<title>` 은 마케팅 문구를 그대로
유지한다. 그래서 title 검사가 통과하고, `raw=0 / csp=0 / err=0` 도 전부 통과한다.
이 세션에서만 "github 정상" 이라고 **세 번** 보고했다 — 전부 이것이었다.

**교훈**: `<title>` 은 페이지가 살아 있다는 증거가 못 된다. SPA/서버 에러 페이지는
타이틀을 유지한 채 본문만 갈아치운다. 본문 분량과 높이를 대조군과 비교해야
"떴다" 를 말할 수 있다.
