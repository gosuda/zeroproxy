...
| 13. Worker containment | Partial | Worker/SharedWorker constructors, data/blob workers, service worker registration blocking, worklet addModule wrapping, worker script URL recovery from module/proxy URLs, worker `fetch`/`importScripts` bridging, worker-side Phase 2 membrane helpers, and worker prelude exist. Worker APIs are not all routed with browser-native fidelity; several are blocked. |
