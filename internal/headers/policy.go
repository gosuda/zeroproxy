...
//
// challengeCompat is the caller-computed two-signal gate result (per-tab arm
// AND classifier match for a challenge SUBRESOURCE, never the document). When
// true the Cache-Control overwrite is SKIPPED so Cloudflare's own cache/update
...
// the proxy transport are untouched — it grants no egress and no eval. When
// false (every existing call path passes false) the no-store overwrite is
// applied exactly as before, keeping the default/OFF path behaviorally
// identical to the pre-armed signature.
