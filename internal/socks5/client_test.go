...
// TestConnectDomainFailsClosedOnHostileReply is the adversarial fail-closed
// table: each case feeds the REAL handshake/parse code a hostile or garbled
// relay reply and asserts (a) the SPECIFIC guard error and (b) that the proxy
// does NOT proceed to tunnel. ConnectDomain never tunnels itself — its caller
// tunnels iff it returns nil — so (b) is: a non-nil error is returned. For
// cases that fail BEFORE the CONNECT request is written, we additionally
// assert the host bytes never reached the relay, an independent witness that
// the handshake never advanced.
func TestConnectDomainFailsClosedOnHostileReply(t *testing.T) {
	tests := []struct {
		...
		{
			// client.go ~:75 — method-selection reply version != 0x05.
		...
		{
			// client.go ~:78 — server selects 0xff "no acceptable method".
		...
		{
			// client.go ~:85 — server selects an auth method the client never
		...
		{
			// client.go ~:130 — username/password auth reply with a nonzero
		...
		{
			// client.go ~:105 — CONNECT reply with nonzero REP code
		...
		{
			// client.go ~:108 — CONNECT reply with nonzero reserved byte.
		...
		{
			// client.go ~:149 — unknown reply ATYP in discardBindAddress.
			// Leading {0x05,0x00} is the method-selection reply; then a CONNECT
