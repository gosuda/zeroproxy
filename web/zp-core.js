...
(() => {
  ...
  // own WebSocket origin, and every normalized relay origin (loopback WS permitted).
  // It adds no wildcard and no direct-egress capability; relay fetches still route
  // through the proxy transport.
  function buildConnectSrc(servers, ws) {
  ...
  function fixedCSP(servers, options = {}) {
    ...
    return "default-src 'none'; " + script + "; style-src * 'unsafe-inline' blob: data:; img-src * blob: data:; font-src * blob: data:; media-src * blob: data:; connect-src " + connectSrc + "; frame-src 'self' blob: data:; child-src 'self' blob: data:; worker-src 'self' blob:; object-src 'none'; base-uri 'none'; form-action 'self'; manifest-src 'self'";
  }
