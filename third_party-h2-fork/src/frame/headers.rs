...
impl Iterator for Iter {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some(ref mut pseudo) = self.pseudo {
            ...
            // ZeroProxy h2 fork: emit `:authority` BEFORE `:scheme`
            // (Chrome 134 wire order). Upstream h2 0.4.14 emits scheme
            // then authority, which lands the Akamai-style H2 fingerprint
            // last field on `m,s,a,p`; Chrome 134 wants `m,a,s,p`.
            // Anti-bot WAFs (NAVER nid.* family) check pseudo-header
            // ordering as part of their H2 fingerprint match.
