...
Status: implemented strict Phase 2 cutover for the default rewrite path. The Service Worker loads the packaged OXC parser WASM and `web/js-rewriter.js` performs AST-aware source rewriting from the OXC AST. The Go WASM HTML transformer calls that rewriter for inline scripts and event handlers, main-window dynamic `Function`/`eval`/string timer bodies execute under the runtime's virtual global scope, external/module/worker/imported script responses are rewritten before execution, worker scripts receive worker-side membrane helpers, and unrewritable blob/data worker paths fail closed. Explicit compatibility passthrough exceptions are documented below.
...
2. **Preserve Service Worker message capabilities**
   ...
   - Current baseline: privileged target-document runtime messages carry a per-tab capability token generated by the Service Worker, embedded in the inert `__zp-boot` JSON script, read into a closure by `runtime-prelude.js`, and kept out of public globals.
...
3. **Close blob/data script execution gaps**
   ...
   - Current status: worker-oriented `URL.createObjectURL` handling wraps JavaScript-like, empty, text/plain, and octet-stream Blob worker bodies with a throwing ZeroProxy worker prelude. `data:` workers are similarly converted to throwing contained workers. Dynamic DOM script insertion is neutralized by the runtime HTML/script hooks and strict CSP, rather than executed as raw blob/data JavaScript.
   - Remaining limitation:
     - Blob MIME detection is still compatibility-oriented, not a full browser MIME/classification model.
     - A future rewritten blob/data execution path must be explicitly designed and tested before enabling raw execution.
     - Revisit `script-src blob: data:` together with the CSP gate above before any high-assurance acceptance claim.
   - Required tests:
...
Target response
  ...
  -> JavaScript Rewriting Service in Service Worker
       -> packaged OXC parser WASM + JavaScript AST rewriter
       -> AST rewrite
...
|---|---|---|
| Rewriter WASM/parser | `@oxc-parser/wasm` assets copied by `scripts/build.mjs` into `dist/web` | Parse JavaScript with OXC for Service Worker rewriting. |
| Service Worker Rewriting Service | `web/sw.js`, `web/js-rewriter.js`, `/__zp/oxc-parser.js`, `/__zp/oxc_parser_wasm_bg.wasm` | Load the parser WASM, rewrite script responses, and enforce the fail-closed policy. |
| Runtime membrane | `web/runtime-prelude.js`, `web/worker-prelude.js` | Provide `__zp_get`, `__zp_set`, `__zp_call`, `__zp_construct`, virtual `Location`, virtual window/worker wrappers, and navigation helpers used by rewritten code. |
| Foreground dynamic-code scoping | `web/runtime-prelude.js` | Wrap synchronous `eval`, `Function`, constructor-constructor, and string timer bodies in the runtime virtual global scope. |
...
| External script pipeline | `web/sw.js` | Rewrite classic scripts, module scripts, worker scripts, and imported worker scripts before returning them to the browser/worker. |
| Recursive runtime policy | `web/runtime-prelude.js`, `web/worker-prelude.js` | Install storage/network/device/iframe containment in accessible window, iframe, and `srcdoc` realms; workers get a smaller prelude with fetch/importScripts bridging, virtual location, blocked high-risk APIs, and membrane helpers. |
...
| Foreground in-place HTML policy | `web/runtime-prelude.js` | Conservatively transform dynamic HTML sinks before browser compilation, launder dynamic script URLs, neutralize inline event attributes, and suppress active preload/preconnect/manifest links while preserving inert link nodes for framework compatibility. |
...
### Foreground synchronous dynamic-code policy

Current code does not ship a separate foreground OXC bundle. Instead, `runtime-prelude.js` installs synchronous wrappers before target scripts run:

1. `eval`, indirect eval, `Function`, constructor-constructor escapes, string timers, and related dynamic constructors execute under `with(__zp_scope)` so reads/writes/calls resolve through the runtime virtual global membrane.
2. Dynamic DOM compilation sinks (`innerHTML`, `outerHTML`, `insertAdjacentHTML`, `document.write`, `template.innerHTML`, `DOMParser.parseFromString`, `Range.createContextualFragment`, and `iframe.srcdoc`) pass through the foreground `transformHTML` policy before insertion.
3. The foreground HTML policy is conservative rather than a full tokenizer pass: it blocks dynamic `<script>` tags, launders dynamic script element URL mutations elsewhere, backs up `integrity`, neutralizes inline event attributes, replaces `<base>`, injects `srcdoc` prelude code, and suppresses active preload/preconnect/manifest links while preserving inert link nodes for framework compatibility.
4. No public `window.__zp_rewrite` downgrade switch is exposed. Target code only reaches the policy through patched browser APIs.
5. If a dynamic source cannot be contained synchronously, strict behavior is to keep it inert or execute it only inside the virtual membrane; original target JavaScript is not executed outside that membrane as a fallback.

Acceptance tests:

- dynamic event handlers, string timers, `Function`, and constructor-constructor code observe virtual `location` and routed network APIs;
- dynamic script/data/blob insertion either routes through the script API / worker bootstrap or remains inert;
