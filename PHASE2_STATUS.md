...
| **E3** Performance baseline | `[~]` | **Bundle sizes (post wasm-opt -Oz, 2026-06-06 refresh)**: `zp_bundle_bg.wasm` 3.38 MB, `zp_bundle_sw_bg.wasm` 3.38 MB (was 3.07 MB; +10% from D2 sourcemap chaining + transport-codec extraction + anchor escape vector backstop). Target ≤ 500 KB — gap is OXC + rustls + h2 + ml-kem + lol_html + sourcemap inherent size. ≥ 5× compression target needs split-bundle or streaming parse — multi-session track. JS wbg glue 44–60 KB (`zp_bundle_sw.js` ≈ 60 KB, near the 50 KB ceiling; `npm run build -- --minify` shaves ~30%). `zp_page_rt.wasm` 15 KB. **Rewriter latency (`cargo bench rewrite_perf --quick`, 2026-06-06)**: 10 KB ≈ 540 µs warm (+43% vs 377 µs baseline), 100 KB ≈ 5.5 ms warm (+28% vs 4.27 ms), 1 MB ≈ 59.8 ms warm (+28% vs 46.5 ms), ~17 MiB/s throughput (-19% vs 21 MiB/s). Regression source = sourcemap chaining glue inside the full-emit path; the SW prefers **patch-mode emit** ([`rewrite_script_patches`](crates/zp-rewriter/src/lib.rs#L161) + [`ZPBundle.rewriteScriptPatches`](crates/zp-bundle/src/lib.rs#L70) + [`sw.js apply-patch-envelope`](web/sw.js#L925-L1000)), so the user-visible path is unaffected — full-emit bench is the cold-fallback measurement. `twiggy garbage` on the SW wasm reports 55 bytes true dead code (wasm-opt -Oz already collected); 32.7% data segments are potential-false-positive but not safely strippable without runtime semantic knowledge. Next single-session perf candidates: (a) OXC feature flag audit (`semantic_builder`, unused codegen options), (b) `lol_html` selector-engine tree-shake (only `element!` macro used), (c) split-bundle skeleton (kernel vs rewriter wasm separation, multi-session). See `Phase 2 follow-up` below. |
